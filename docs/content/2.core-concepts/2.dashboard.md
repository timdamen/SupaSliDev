...
The Supaslidev dashboard provides a visual interface and CLI commands for managing all your presentations from one place. It handles starting dev servers, creating new presentations, exporting to PDF, and preparing deployment packages.
...
This is the default command. Running `pnpm dashboard` without arguments launches the full dashboard interface.
...
```bash
pnpm dashboard export my-talk --output ./exports/presentation.pdf
```
...
```bash
pnpm dashboard deploy my-talk --output ./my-deploy-folder
```
...
| Command | Description |
|---------|-------------|
| `pnpm dashboard` | Launch interactive dashboard UI |
| `pnpm dashboard dev <name>` | Start dev server for one presentation |
| `pnpm dashboard create [name]` | Create a new presentation |
| `pnpm dashboard export <name>` | Export presentation to PDF |
| `pnpm dashboard deploy <name>` | Build deployment package |

## Next Steps

Learn about [creating presentations](/guide/creating-presentations) or explore [deployment options](/guide/deployment) for hosting your slides.
